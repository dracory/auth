...
- [x] **Hardcoded cookie name (fixed)** — `types.CookieName` is now a `var` (default `"authtoken"`) for backward-compatible global overrides; it is documented as init-time-only, with per-instance customization preferred via `CookieConfig.Name`/`SetCookieName`. `authImplementation` stores a `cookieName` field with `GetCookieName()`/`SetCookieName()` accessors exposed on `types.AuthSharedInterface`. `NewUsernameAndPasswordAuth` and `NewPasswordlessAuth` now merge `config.CookieConfig` onto `defaultCookieConfig()` so setting only `CookieConfig.Name` preserves `HttpOnly` and `Secure` defaults. Cookie set/remove helpers in `utils` and `auth` packages use `cfg.Name` when provided. `utils.AuthTokenRetrieve` and `utils.AuthCookieGet` gain `WithName` variants that accept a cookie name, and all middleware (`WebAuthOrRedirectMiddleware`, `WebAppendUserIdIfExistsMiddleware`, `ImpersonationAwareMiddleware`, `ApiAuthOrErrorMiddleware`) and API handlers (`api_logout`, `api_impersonate_start`, `api_impersonate_stop`) use the instance-specific cookie name. This allows multiple auth instances on the same origin to use independent cookie namespaces. Regression tests: `TestAuthCookieSet_WithCookieName`, `TestAuthCookieRemove_WithCookieName`, `TestAuthCookieGetWithName_*`, `TestAuthTokenRetrieveWithName_*` in `utils/*_test.go`; `TestGetCookieName_*`, `TestSetCookieName`, `TestSetAuthCookie_UsesCookieName`, `TestRemoveAuthCookie_UsesCookieName` in `auth_implementation_test.go`; `TestNewUsernameAndPasswordAuth_CookieConfigName` and `TestNewPasswordlessAuth_CookieConfigName` (asserting `HttpOnly`/`Secure` defaults) in `new_*_auth_test.go`; `TestApiAuthOrErrorMiddleware_CustomCookieName` in `middlewares/api_auth_or_error_middleware_test.go`.
